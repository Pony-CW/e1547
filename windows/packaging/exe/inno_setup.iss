...
english.RemoveUserData=Remove user data from the following location?%n%n%1

chinesesimplified.CreateStartMenuIcon=创建开始菜单快捷方式
chinesesimplified.RemoveUserData=是否删除以下位置的用户数据？%n%n%1

[Tasks]
Name: "desktopicon"; Description: "{cm:CreateDesktopIcon}"; GroupDescription: "{cm:AdditionalIcons}"; Flags: {% if CREATE_DESKTOP_ICON != true %}unchecked{% else %}checkedonce{% endif %}
Name: "startmenuicon"; Description: "{cm:CreateStartMenuIcon}"; GroupDescription: "{cm:AdditionalIcons}"; Flags: {% if CREATE_START_MENU_ICON != true %}unchecked{% else %}checkedonce{% endif %}
[Files]
Source: "{{SOURCE_DIR}}\\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
; NOTE: Don't use "Flags: ignoreversion" on any shared system files

[Icons]
Name: "{autoprograms}\\{{DISPLAY_NAME}}"; Filename: "{app}\\{{EXECUTABLE_NAME}}"; Tasks: startmenuicon
...
begin
  ...
  begin
    ...
    begin
      MsgText := FmtMessage(ExpandConstant('{cm:RemoveUserData}'), [AppDataDir]);
      RemoveUserData := MsgBox(MsgText, mbConfirmation, MB_YESNO) = IDYES;
    end
    else
      RemoveUserData := False;
  end;

  if (CurStep = usPostUninstall) and RemoveUserData then
    if DirExists(AppDataDir) then
